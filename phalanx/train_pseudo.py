...
parser.add_argument('--pseudo_path', default='../input/pseudolabels_v2/', type=float, help='pseudo labels path')
...
if __name__ == '__main__':
    ...
    train_data = SaltDataset(image_train,mask_train, is_train=True, fine_size=args.fine_size, pad_left=args.pad_left, pad_right=args.pad_right)
    val_data = SaltDataset(image_val,mask_val, is_val=True, fine_size=args.fine_size, pad_left=args.pad_left, pad_right=args.pad_right)
    ...
    for epoch in mb:
        ...
        if (epoch+1) % scheduler_step == 0:
            optimizer = torch.optim.SGD(salt.parameters(), lr=args.lr, momentum=args.momentum,  weight_decay=args.weight_decay)
            lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, scheduler_step, args.min_lr)
        ...
        mb.write('epoch: {} train_loss: {:.3f} test_loss: {:.3f} accuracy: {:.3f}'.format(epoch+1, train_loss, val_loss, precision))
