...
class SaltDataset(Dataset):
    def __init__(self, image_list, mask_list=None, is_train=False, is_val=False, is_tta=False, is_semi=False, fine_size=202, pad_left=0, pad_right=0):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            image = image.reshape(1, self.fine_size+self.pad_left+self.pad_right, self.fine_size+self.pad_left+self.pad_right)
            mask = mask.reshape(1, self.fine_size+self.pad_left+self.pad_right, self.fine_size+self.pad_left+self.pad_right)
...
def trainImageFetch(images_id):
    ...
    for idx, image_id in tqdm(enumerate(images_id), total=images_id.shape[0]):
        image_path = '../input/train/images/' + image_id + '.png'
        mask_path = '../input/train/masks/' + image_id + '.png'
...
def semi_trainImageFetch(pseudo_path):
    ...
    for idx, image_id in tqdm(enumerate(images_id)):
        image_path = '../input/test/images/' + image_id
...
def testImageFetch():
    ...
    for n, image_id in tqdm(enumerate(test_id), total=len(test_id)):
        image_path = '../input/test/images/' + image_id + '.png'
